...
class InventoryController(
    ...
    @ResponseStatus(HttpStatus.CREATED)
    @Operation(
        summary = "Crear un nuevo rollo de tela",
        description = "Dado un nombre y un color, genera un rollo de tela"
    )
    @ApiResponses(
        value = [
    ...
    fun updateGarment(
        @RequestBody updateGarmentRequest: UpdateGarmentRequest,
    ...
    fun updateFabricRoll(
        @RequestBody request: UpdateFabricRollRequest,
        @PathVariable("id") id: UUID
    ): ResponseEntity<GenericResponse<Any>> {
    ...
    fun createColor(
        @Valid @RequestBody createColorRequest: CreateColorRequest,
        @AuthenticationPrincipal jwt: Jwt
    ): ResponseEntity<GenericResponse<Any>> {
        val user = validateUser(jwt, usuarioRepository)
    ...
    fun updateFabricPiece(
        @RequestBody updateGarmentRequest: UpdateFabricPieceRequest,
        @PathVariable("id") id: UUID
    ): ResponseEntity<GenericResponse<Any>> {
    ...
    @DeleteMapping("/rollo/{id}")
    @ResponseStatus(HttpStatus.OK)
    @Operation(summary = "Eliminar un rollo")
    @ApiResponses(
        value = [
            ApiResponse(description = "Rollo eliminado correctamente", responseCode = "204"),
            ApiResponse(description = "Rollo no encontrado", responseCode = "404")
        ]
    )
    fun deleteFabricRoll(@PathVariable("id") id: UUID): ResponseEntity<GenericResponse<Any>> {
        return try {
            inventoryService.deleteFabricRoll(id)
            ResponseEntity.noContent().build()
        } catch (ex: FabricRollNotFoundException) {
    ...
    @DeleteMapping("/fabricPiece/{id}")
    @ResponseStatus(HttpStatus.OK)
    @Operation(summary = "Eliminar un molde cortado")
    @ApiResponses(
        value = [
            ApiResponse(description = "Molde cortado eliminado correctamente", responseCode = "204"),
            ApiResponse(description = "Molde cortado no encontrado", responseCode = "404")
        ]
    )
    fun deleteFabricPiece(@PathVariable("id") id: UUID): ResponseEntity<GenericResponse<Any>> {
        return try {
            inventoryService.deleteFabricPiece(id)
            ResponseEntity.noContent().build()
        } catch (ex: FabricPieceNotFoundException) {
            ResponseEntity.status(HttpStatus.NOT_FOUND).body(
                GenericResponse(
                    status = "fail",
                    data = mapOf("exception" to ex.javaClass.simpleName, "message" to ex.message)
    ...
    @ApiResponses(
        value = [
            ApiResponse(description = "Prenda eliminada correctamente", responseCode = "204"),
            ApiResponse(description = "Prenda no encontrada", responseCode = "404")
        ]
    )
    fun deleteGarment(@PathVariable("id") id: UUID): ResponseEntity<GenericResponse<Any>> {
        return try {
            inventoryService.deleteGarment(id)
            ResponseEntity.noContent().build()
        } catch (ex: GarmentNotFoundException) {
            ResponseEntity.status(HttpStatus.NOT_FOUND).body(
                GenericResponse(
                    status = "fail",
                    data = mapOf("exception" to ex.javaClass.simpleName, "message" to ex.message)
